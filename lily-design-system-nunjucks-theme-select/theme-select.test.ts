...
function setup(
    ...
    const controller = initThemeSelect(root, initOpts);
...
describe("ThemeSelect — macro markup contract (§7.1–§7.6)", () => {
    ...
    test("§7.1 the button renders the half-circle glyph, hidden from assistive tech", () => {
        ...
        const icon = root.querySelector(".theme-select-icon") as HTMLElement;
        expect(icon.textContent).toBe(CIRCLE_WITH_RIGHT_HALF_BLACK);
        expect(icon.getAttribute("aria-hidden")).toBe("true");
        // The glyph must never be the accessible name.
        expect(root.querySelector(".theme-select-button")!.textContent).not.toBe(
            root
...
describe("ThemeSelect — server-rendered listbox state (§7.14–§7.16)", () => {
...
describe("ThemeSelect — keyboard contract (APG listbox, §7.20–§7.24)", () => {
...
describe("ThemeSelect — system-preference detection (§7.27)", () => {
    ...
    test("§7.27 the macro emits the detect flag; detection never affects server markup", () => {
        ...
        expect(
            root.getAttribute("data-lily-theme-select-detect-from-system"),
        ).toBe("true");
...
describe("ThemeSelect — initial-value precedence (§7.28)", () => {
...
describe("ThemeSelect — client.js lifecycle (§7.7–§7.11, §7.13)", () => {
    ...
    test("§7.17 opts.value is carried on data-lily-theme-select-value and resolves the initial theme", () => {
        const root = mountIntoBody(
            renderMacro({
                label: "Theme",
                themesUrl: URL_TRAILING,
    ...
    test("autoInit wires every root on the page", () => {
        ...
        // do not collide.
        const lists = document.querySelectorAll(".theme-select-list");
