...
//      `data-lily-theme-select-value`. This is still the only channel
 ...
 *
 * Mirrors `localeName` in locale-select. This is the JS statement of the
...
/**
 * Wire one rendered ThemeSelect root.
 *
 * @param {HTMLElement} root - The <div data-lily-theme-select-root>.
...
export function initThemeSelect(root, opts = {}) {
    ...
    const button = root.querySelector("[data-lily-theme-select-button]");
    const list = root.querySelector("[data-lily-theme-select-list]");
    const input = root.querySelector("[data-lily-theme-select-input]");
    ...
    // 1. value prop — read from `data-lily-theme-select-value`.
    initial = valueAttr;
