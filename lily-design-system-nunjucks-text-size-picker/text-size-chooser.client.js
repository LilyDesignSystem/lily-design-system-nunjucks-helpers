// TextSizeChooser client-side runtime.
//
// Pairs with text-size-chooser.njk. The macro renders the markup with
// `data-lily-text-size-chooser-*` hooks; this module picks them up in
...
//      `data-lily-text-size-chooser-value`. This is the only channel by
...
// `prefers-color-scheme` (theme-chooser) and `navigator.languages`
// (locale-chooser), the web platform exposes no OS "preferred text
// size" signal.
//
 ...
 * locale-chooser. This is the JS statement of the rule the macro applies
...
/**
 * Wire one rendered TextSizeChooser root.
 *
 * @param {HTMLElement} root - The <div data-lily-text-size-chooser-root>.
 * @param {{onChange?: (size:string)=>void, target?: HTMLElement|null}=} opts
 * @returns {{setSize: (size: string) => void, destroy: () => void}}
 */
export function initTextSizeChooser(root, opts = {}) {
    const noop = { setSize: () => {}, destroy: () => {} };
    if (typeof document === "undefined" || !root) return noop;

    const button = root.querySelector("[data-lily-text-size-chooser-button]");
    const list = root.querySelector("[data-lily-text-size-chooser-list]");
    const input = root.querySelector("[data-lily-text-size-chooser-input]");
    ...
    const storageKey =
        root.getAttribute("data-lily-text-size-chooser-storage-key") || "";
    const defaultValue =
        root.getAttribute("data-lily-text-size-chooser-default-value") || "";
    ...
    const valueAttr =
        root.getAttribute("data-lily-text-size-chooser-value") || "";
    ...
    // storage here, so unlike theme-chooser there is no precedence
    // reversal to warn about.
    // -----------------------------------------------------------------

    let initial = "";

    // 1. value prop — read from `data-lily-text-size-chooser-value`.
    initial = valueAttr;
...
/**
 * Find every [data-lily-text-size-chooser-root] and wire it.
 *
 * @param {{onChange?: (size:string)=>void, target?: HTMLElement|null}=} opts
...
export function autoInit(opts = {}) {
    ...
    const roots = Array.from(
        document.querySelectorAll("[data-lily-text-size-chooser-root]"),
    );
    return roots.map((root) => initTextSizeChooser(root, opts));
}
