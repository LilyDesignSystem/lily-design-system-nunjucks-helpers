...
// ---------------------------------------------------------------------
// Nunjucks env that can resolve `./text-size-picker.njk` from this dir.
// ---------------------------------------------------------------------
...
function renderMacroWithCaller(
  ...
  const src =
    `{% from "./text-size-picker.njk" import textSizePicker %}` +
    `{% call textSizePicker(opts) %}${body}{% endcall %}`;
  return env.renderString(src, { opts });
}
...
function partsOf(root: HTMLElement): Parts {
  return {
    ...
    list: root.querySelector(".text-size-picker-list") as HTMLElement,
    options: Array.from(
      root.querySelectorAll<HTMLElement>(".text-size-picker-option"),
    ),
...
function setup(
  ...
  const controller = initTextSizePicker(root, initOpts);
...
describe("TextSizePicker — macro markup contract (§7.1–§7.6)", () => {
...
describe("TextSizePicker — server-rendered listbox state (§7.14–§7.16)", () => {
...
describe("TextSizePicker — keyboard contract (APG listbox, §7.20–§7.24)", () => {
...
describe("TextSizePicker — pure helpers (§7.25)", () => {
...
describe("TextSizePicker — initial-value resolution (§7.26–§7.28)", () => {
...
describe("TextSizePicker — client.js lifecycle (§7.7–§7.13, §7.17–§7.19)", () => {
  ...
  test("§7.17 opts.value is carried on data-lily-text-size-picker-value and resolves the initial size", () => {
    const root = mountIntoBody(
      renderMacro({
        label: "Text size",
        sizes: SIZES,
    ...
    expect(root.getAttribute("data-lily-text-size-picker-value")).toBe(
      "x-large",
    );
    initTextSizePicker(root);
    expect(document.documentElement.dataset.textSize).toBe("x-large");
  });
  ...
  test("§7.18 the value data attribute is omitted entirely when opts.value is unset", () => {
    ...
    expect(root.hasAttribute("data-lily-text-size-picker-value")).toBe(false);
  });
  ...
  test("§7.19 a call block replaces the glyph inside the button", () => {
    ...
    expect(button.querySelector(".text-size-picker-icon")).toBeNull();
  ...
  test("§7.11 autoInit wires every root on the page", () => {
    ...
    // do not collide.
    const lists = document.querySelectorAll(".text-size-picker-list");
    expect(lists[0].id).not.toBe(lists[1].id);
  });

  test("§7.12 init is a no-op on a root missing its button and list", () => {
    document.body.innerHTML = `<div data-lily-text-size-picker-root></div>`;
    const root = document.body.firstElementChild as HTMLElement;
    const controller = initTextSizePicker(root);
    expect(() => controller.setSize("large")).not.toThrow();
    expect(document.documentElement.hasAttribute("data-text-size")).toBe(false);
  });
});

describe("TextSizePicker — accessibility hardening (§7.29–§7.32; canonical Svelte §7.14–§7.17)", () => {
  function openPicker(
    sizes: string[] = SIZES,
    extra: Record<string, unknown> = {},
  ) {
    const { button, list } = setup({ sizes, ...extra });
    click(button);
    return { button, list };
  }
  ...
  test("§7.30 a repeated typeahead character cycles through its matches", () => {
    const { list } = openPicker(["d1", "d2", "d3", "m"], {
