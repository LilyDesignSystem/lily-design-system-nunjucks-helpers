...
`locale-select.client.js` rather than by the platform. The canonical
...
| `<span class="locale-select-icon">` | `aria-hidden="true"`             | Macro         |
...
Implemented in `locale-select.client.js`. None of it exists before
that module runs.
...
listbox is opened. The compensating `.locale-select-status` region
...
button opens nothing until `locale-select.client.js` runs, because
