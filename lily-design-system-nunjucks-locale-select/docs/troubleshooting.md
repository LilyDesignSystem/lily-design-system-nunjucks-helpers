...
**Likely cause.** `locale-select.client.js` has not run. The control is
...
- Confirm a `<script type="module">` on the page imports the client and
  calls `autoInit()` (or `initLocaleSelect(root)`).
...
**Likely cause.** `locale-select.client.js` imports `./locales.js` for
...
```js
import { isRtlLocale } from "./locale-select.client.js";
...
prefix `locale-select-{name}` — duplicate listbox and option ids, and
...
`.locale-select-option`, and avoid a fixed `line-height` that clips
