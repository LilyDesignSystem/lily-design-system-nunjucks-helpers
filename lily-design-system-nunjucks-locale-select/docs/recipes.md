...
```njk
{{ localeSelect({ label: "Language", locales: LOCALES, localeLabels: NATIVE }) }}
...
```js
import { autoInit, bcp47LocaleTag } from "./locale-select.client.js";
...
`target` to `initLocaleSelect` to scope them to a region instead —
...
initLocaleSelect(document.querySelector("#preview-switcher"), {
...
```njk
{{ localeSelect({ label: "Language", locales: LOCALES, applyDir: false }) }}
```

```js
import { autoInit, isRtlLocale } from "./locale-select.client.js";
...
```js
import { autoInit, matchNavigatorLanguage, localeName } from "./locale-select.client.js";
...
```njk
{{ localeSelect({ label: "Language", locales: LOCALES, name: "locale-header" }) }}
{{ localeSelect({ label: "Language", locales: LOCALES, name: "locale-footer" }) }}
```
