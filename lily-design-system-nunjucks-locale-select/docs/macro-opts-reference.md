...
Field-by-field reference for every key the `localeSelect(opts)` macro
...
`.locale-select-status` region that compensates for it.

`label` carries a wrinkle the theme select does not have. It should
...
`.locale-select-placeholder` CSS hook.
...
`data-lily-locale-select-value` on the root `<div>`, and omits the
...
is also the default id prefix (`locale-select-{name}`), which is why two
...
## `id` — optional, string, default `"locale-select-{name}"`
...
Extra classes appended to the root `<div>` after the `locale-select`
