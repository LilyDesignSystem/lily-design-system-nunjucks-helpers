...
theme-select's monochrome ◑, and at a different optical size and
...
Owned entirely by `locale-select.client.js`. Nothing below works
...
- **Styling the list open by default.** The `hidden` attribute is the
  open-state contract; a CSS rule like `.locale-select-list { display:
  block }` overrides `hidden` and leaves the list permanently visible
  and out of sync with `aria-expanded`. Use
  `.locale-select-list:not([hidden])` for open-state styling.
