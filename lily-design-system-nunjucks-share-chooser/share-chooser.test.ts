...
// ---------------------------------------------------------------------
// Nunjucks env that can resolve `./share-chooser.njk` from this dir.
// ---------------------------------------------------------------------
...
function renderMacroWithCaller(
    ...
    const src =
        `{% from "./share-chooser.njk" import shareChooser %}` +
        `{% call shareChooser(opts) %}${body}{% endcall %}`;
    return env.renderString(src, { opts });
}
...
type Parts = {
    ...
    api: ReturnType<typeof initShareChooser>;
};
...
function setup(
    ...
    const api = initShareChooser(root, initOpts);
    return {
        root,
        trigger: root.querySelector(
            ".share-chooser-button",
        ) as HTMLButtonElement,
        list: root.querySelector(".share-chooser-list") as HTMLElement,
        status: root.querySelector(".share-chooser-status") as HTMLElement,
        targets: Array.from(
            root.querySelectorAll<HTMLAnchorElement>(".share-chooser-target"),
        ),
        copy: root.querySelector(".share-chooser-copy"),
        api,
    };
}
...
describe("ShareChooser — markup contract (§7.1–§7.6)", () => {
    ...
    test("§7.1 the trigger class is share-chooser-button, matching the other helpers", () => {
        const { root, trigger } = setup();
        expect(trigger.className).toBe("share-chooser-button");
        expect(root.querySelector(".share-chooser-button")).toBe(trigger);
        // The pre-rename `share-button-trigger` exception is gone.
        expect(root.querySelector(".share-chooser-trigger")).toBeNull();
    });
    ...
    test("§7.3 each destination sits in its own list item", () => {
        ...
        const lis = root.querySelectorAll(".share-chooser-list-item");
        expect(lis.length).toBe(2);
        for (const a of targets) {
            expect(a.parentElement?.className).toBe("share-chooser-list-item");
...
describe("ShareChooser — url resolution and glyph (§7.20–§7.22)", () => {
    ...
    test("§7.20 the url travels on a data attribute, not a rendered href", () => {
        ...
        expect(root.getAttribute("data-lily-share-chooser-url")).toBe(
            URL_UNDER_TEST,
        );
        ...
        expect(bare.hasAttribute("data-lily-share-chooser-url")).toBe(false);
    });
    ...
    test("§7.22 a {% call %} body replaces the glyph", () => {
        ...
        expect(root.querySelector(".share-chooser-icon")).toBeNull();
    });
...
describe("ShareChooser — Nunjucks surface (§7.23–§7.26)", () => {
    test("§7.23 ids are deterministic and derived from the default name", () => {
        const { trigger, list } = setup();
        expect(list.id).toBe("share-chooser-share-list");
    ...
    test("§7.23 the name opt drives the ids", () => {
        ...
        expect(list.id).toBe("share-chooser-footer-list");
        expect(root.getAttribute("data-lily-share-chooser-name")).toBe(
            "footer",
        );
        expect(
            root.querySelector(".share-chooser-target")!.id,
        ).toBe("share-chooser-footer-target-0");
    });
    ...
    test("§7.23 an explicit id opt overrides the name-derived prefix", () => {
        ...
        expect(root.querySelector(".share-chooser-copy")!.id).toBe(
            "custom-prefix-copy",
        );
    });
    ...
    test("§7.23 two instances sharing a name collide, distinct ids do not", () => {
        ...
        const lists = Array.from(
            document.querySelectorAll(".share-chooser-list"),
        ).map((l) => l.id);
        expect(new Set(lists).size).toBe(2);
        expect(lists).toEqual(["share-chooser-share-list", "b-list"]);
    });
    ...
    test("§7.26 autoInit wires every root on the page", () => {
        ...
        click(triggers[1]);
        const lists = Array.from(
            document.querySelectorAll<HTMLElement>(".share-chooser-list"),
        );
        expect(lists[0].hasAttribute("hidden")).toBe(true);
        expect(lists[1].hasAttribute("hidden")).toBe(false);
    });

    test("§7.26 initShareChooser is inert on a missing or foreign root", () => {
        expect(() => initShareChooser(null as any)).not.toThrow();
        document.body.innerHTML = "<div></div>";
        const api = initShareChooser(
    ...
    test("§7.26 nextShareChooserId mints stable, incrementing, SSR-safe ids", () => {
...
describe("ShareChooser — no-JS degradation (§6)", () => {
    test("§6 destination links are complete in the server markup", () => {
        // No initShareChooser call at all — this is the no-JS document.
        const root = mountIntoBody(
            renderMacro({
                label: "Share",
                targets: TARGETS,
    ...
    test("§6 the list stays hidden and copy is inert without the client", () => {
        ...
        const list = root.querySelector(".share-chooser-list") as HTMLElement;
        click(trigger);
        expect(list.hasAttribute("hidden")).toBe(true);
        expect(trigger.getAttribute("aria-expanded")).toBe("false");
    });
