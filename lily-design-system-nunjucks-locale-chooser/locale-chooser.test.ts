...
function setup(
    ...
    const controller = initLocaleChooser(root, initOpts);
...
describe("LocaleChooser — macro markup contract (§7.1–§7.6)", () => {
    ...
    test("§7.1 the button renders the globe glyph, hidden from assistive tech", () => {
        ...
        const icon = root.querySelector(".locale-chooser-icon") as HTMLElement;
...
describe("LocaleChooser — server-rendered listbox state (§7.28–§7.30)", () => {
...
describe("LocaleChooser — keyboard contract (APG listbox, §7.31–§7.35)", () => {
...
describe("LocaleChooser — client.js lifecycle (§7.13–§7.17)", () => {
...
describe("LocaleChooser — initial-value resolution (§7.18–§7.21, §7.26–§7.27)", () => {
    ...
    test("§7.19 a supplied non-empty value (carried on the data attribute) wins over storage and defaults", () => {
        ...
        expect(root.getAttribute("data-lily-locale-chooser-value")).toBe("en");
    ...
    test("§7.26 the value data attribute is the sole channel for opts.value", () => {
        ...
        expect(root.getAttribute("data-lily-locale-chooser-value")).toBe("fr");
        initLocaleChooser(root);
        expect(document.documentElement.lang).toBe("fr");
    });
    ...
    test("§7.27 the value data attribute is omitted entirely when opts.value is unset", () => {
        ...
        expect(root.hasAttribute("data-lily-locale-chooser-value")).toBe(false);
    });
...
describe("LocaleChooser — spread, caller, autoInit (§7.22–§7.25)", () => {
