...
// ---------------------------------------------------------------------
// Nunjucks env that can resolve `./text-size-select.njk` from this dir.
// ---------------------------------------------------------------------
...
function renderMacroWithCaller(
    ...
    const src =
        `{% from "./text-size-select.njk" import textSizeSelect %}` +
        `{% call textSizeSelect(opts) %}${body}{% endcall %}`;
    return env.renderString(src, { opts });
}
...
function partsOf(root: HTMLElement): Parts {
    return {
        ...
        list: root.querySelector(".text-size-select-list") as HTMLElement,
        options: Array.from(
            root.querySelectorAll<HTMLElement>(".text-size-select-option"),
        ),
...
function setup(
    ...
    const controller = initTextSizeSelect(root, initOpts);
...
describe("TextSizeSelect — macro markup contract (§7.1–§7.6)", () => {
    ...
    test("§7.1 the button renders the 'A' glyph, hidden from assistive tech", () => {
        ...
        const button = root.querySelector(".text-size-select-button")!;
...
describe("TextSizeSelect — server-rendered listbox state (§7.14–§7.16)", () => {
...
describe("TextSizeSelect — keyboard contract (APG listbox, §7.20–§7.24)", () => {
...
describe("TextSizeSelect — pure helpers (§7.25)", () => {
...
describe("TextSizeSelect — initial-value resolution (§7.26–§7.28)", () => {
...
describe("TextSizeSelect — client.js lifecycle (§7.7–§7.13, §7.17–§7.19)", () => {
    ...
    test("§7.17 opts.value is carried on data-lily-text-size-select-value and resolves the initial size", () => {
        const root = mountIntoBody(
            renderMacro({
                label: "Text size",
                sizes: SIZES,
        ...
        expect(root.getAttribute("data-lily-text-size-select-value")).toBe(
            "x-large",
        );
    ...
    test("§7.18 the value data attribute is omitted entirely when opts.value is unset", () => {
        ...
        expect(root.hasAttribute("data-lily-text-size-select-value")).toBe(
            false,
        );
    });
    ...
    test("§7.19 a call block replaces the glyph inside the button", () => {
        ...
        expect(button.querySelector(".text-size-select-icon")).toBeNull();
    ...
    test("§7.11 autoInit wires every root on the page", () => {
        ...
        // do not collide.
        const lists = document.querySelectorAll(".text-size-select-list");
        expect(lists[0].id).not.toBe(lists[1].id);
    });

    test("§7.12 init is a no-op on a root missing its button and list", () => {
        document.body.innerHTML = `<div data-lily-text-size-select-root></div>`;
        const root = document.body.firstElementChild as HTMLElement;
        const controller = initTextSizeSelect(root);
