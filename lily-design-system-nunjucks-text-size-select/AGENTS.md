...
styles the `text-size-select` class hooks and maps each
...
that opens a listbox, matching `theme-select` and `locale-select`.
...
| `text-size-select.client.js`  | ES module — `initTextSizeSelect`, `autoInit`, `sizeName`, glyph. |
...
- `import { initTextSizeSelect, autoInit, sizeName, LATIN_CAPITAL_LETTER_A } from "./text-size-select.client.js"`
- `sizeName(slug)` mirrors theme-select's `themeName` and
  locale-select's `localeName`: `"x-large"` → `"X Large"`. It is the
...
- Required call: `initTextSizeSelect(rootElement, opts?)` or
  `autoInit(opts?)` to wire every `[data-lily-text-size-select-root]`
  on the page.

## Behaviour contract (one paragraph)

The macro emits a `<div class="text-size-select">` carrying
`data-lily-text-size-select-*` hooks describing the control's name,
storage key, default value, and — when `opts.value` is set — the
consumer's initial value (`data-lily-text-size-select-value`). Inside
it are a hidden input, an icon `<button>`, and a
`<ul role="listbox" hidden>`. On `initTextSizeSelect(root)`, the client
...
<div class="text-size-select {classes}" data-lily-text-size-select-root …>
  ...
  <ul class="text-size-select-list" id="{id}-list" role="listbox"
    ...
    <li class="text-size-select-option" id="{id}-option-{i}" role="option"
...
ONLY on `data-lily-text-size-select-value`.

Ids are `{id}-list` / `{id}-option-{i}` where `id` defaults to
`text-size-select-{name}`. Deterministic and SSR-safe; two instances
