...
`lily-design-system-nunjucks-locale-picker`. Each file is a
...
- The macro file (`../locale-picker.njk`) and the client.js
  (`../locale-picker.client.js`) are reachable from your
...
- The consumer styles the `locale-picker` CSS class hook. The
  helper ships zero CSS.

| #  | File                                                              | Demonstrates                                                       |
|----|-------------------------------------------------------------------|--------------------------------------------------------------------|
| 1  | [`01-radios.njk`](./01-radios.njk)                                | Default radios via `{{ localePicker({...}) }}`.                    |
| 2  | [`02-select.njk`](./02-select.njk)                                | Custom-rendering via `{% call %}` block emitting a `<select>`.     |
| 3  | [`03-buttons.njk`](./03-buttons.njk)                              | Button group + `aria-pressed`.                                     |
| 4  | [`04-rtl-demo.njk`](./04-rtl-demo.njk)                            | RTL locales (ar, he, fa, ur, ps) showing `dir` flipping.           |
| 5  | [`05-nhs-style.njk`](./05-nhs-style.njk)                          | Kebab-case `locale-picker` class + NHS-style language banner.      |
...
|    | [`localePickerCustom.njk`](./localePickerCustom.njk)              | Forked helper macro for the caller-block examples.                 |
...
2. Serve the static-asset directory containing
   `locale-picker.client.js` and `locales.js` (the compiled JS
   form of the `locales.ts` source).
3. Visit the route. The macro renders the radio group; the
...
`initLocalePicker(root, opts)` (and on `autoInit(opts)`),
...
The shipped `locale-picker.njk` does not inspect `caller` — the
default rendering is fixed. Examples 02, 03, 05, and 10 use a
forked macro [`localePickerCustom.njk`](./localePickerCustom.njk)
that replaces the per-option body with `{{ caller() }}`. Copy
this fork into your project when you need custom rendering.

The fork preserves the outer `<fieldset role="radiogroup">` with
all the `data-lily-locale-picker-*` hooks, so the client.js's
`initLocalePicker(root)` still wires the lifecycle. Your caller
block renders whatever markup you want (select, buttons,
combobox, swatch grid); you wire click / change events to
...
- CSS class hooks are kebab-case: `locale-picker`,
  `locale-picker-option`, `locale-picker-option-label`.
- `data-lily-locale-picker-*` attributes are kebab-case
  throughout.
...
- [../docs/accessibility.md](../docs/accessibility.md) —
  WCAG 2.2 AAA + ARIA APG Radio Group implementation.
