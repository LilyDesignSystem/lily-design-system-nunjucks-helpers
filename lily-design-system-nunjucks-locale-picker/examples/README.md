...
- **The control needs JavaScript.** The macro renders an icon button
  ...
  `locale-picker.client.js` runs. Every example therefore loads the
...
theme-picker catalog's convention.
...
|     | [`localePickerCustom.njk`](./localePickerCustom.njk)     | Reusable wrapper macro that swaps the glyph via `{% call %}`.                  |
...
Calling `localePicker` with a `{% call %}` block replaces the default
...
[`localePickerCustom.njk`](./localePickerCustom.njk) packages that
...
DOM with the same hooks — `data-lily-locale-picker-root`, `-button`,
`-list`, `-input`, plus `role="option"` and `data-value` on each
choice. `initLocalePicker(root)` works against any conforming DOM and
...
`"locale-picker-{name}"`. Two instances on one page that share a
...
- [../docs/macro-opts-reference.md](../docs/macro-opts-reference.md) —
  every `localePicker(opts)` key, field by field.
