...
The macro emits a `<div class="locale-picker">` with
`data-lily-locale-picker-*` hooks describing the control's name,
storage key, default value, navigator flag, apply-dir flag, and — when
`opts.value` is set — the consumer's initial value
(`data-lily-locale-picker-value`). Inside it are a hidden input, an
icon `<button>`, and a `<ul role="listbox" hidden>`. On
`initLocalePicker(root)`, the client
...
```html
<div class="locale-picker {classes}" data-lily-locale-picker-root …>
  <input
    type="hidden"
    name="{name}"
    value="{selected}"
...
`data-lily-locale-picker-value`.

Ids are `{id}-list` / `{id}-option-{i}` where `id` defaults to
`locale-picker-{name}`. Deterministic and SSR-safe; two instances
sharing a `name` need an explicit distinct `id`.

There is **no** `placeholder` param and **no**
`.locale-picker-placeholder` hook — both were removed with the
`<select>`.
