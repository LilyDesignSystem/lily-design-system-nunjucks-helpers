...
matching radio gets `checked` rendered server-side and the
`<label lang="fr">` carries the BCP 47 tag.
...
The macro outputs a complete fieldset with the `fr` radio
checked; no DOM touched, no errors thrown.
...
- The server renders no `checked` (because `opts.value=""`),
  but the client picks a non-empty value from `localStorage`.
  The user briefly sees an unchecked group.
