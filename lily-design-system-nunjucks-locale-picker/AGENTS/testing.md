...
```ts
const labels = root.querySelectorAll("label.locale-picker-option");
...
```ts
const fr = root.querySelector('input[value="fr"]') as HTMLInputElement;
fr.checked = true;
...
| §7 group        | Test focus                                                                |
| --------------- | ------------------------------------------------------------------------- |
| 7.1 — 7.6       | Macro DOM contract (fieldset, role, value, name, per-option `lang`).      |
...
```ts
test("§7.16 selecting a radio updates lang and dir …", … );
```
