...
- The radio buttons' `checked` state (so a `<form>` submission also
...
3. **Escape hatch is one caller block away**. The macro's body is
   ...
   [examples/02-select.njk](../examples/02-select.njk) and
   [examples/10-combobox.njk](../examples/10-combobox.njk).

For long locale lists (>~12), use a custom-rendered `<select>` or
combobox. For 2–8 entries, the default radio group is best.
...
losslessly through the radio's `value` attribute, the `onChange`
...
- The client.js writes synchronously to `localStorage` on every
  apply.
- On a fresh mount with no `value` opt (no radio rendered
  `checked`), the stored value is read back.
- Storage errors (private mode, quota) are swallowed silently; the
  picker degrades to the navigator / default / `"en"` / first-radio
  fallback chain.
...
Pass the cookie value as `opts.value` so the matching radio is
rendered `checked` server-side. See [./ssr.md](./ssr.md).
...
2. **Macro output** — `nunjucks.renderString(template, ctx)` and
   assert that the HTML contains the expected radios, hooks, and
   `checked` flag for the seeded `value`.
...
ships with the same shape: a macro that emits a `<fieldset
role="radiogroup">` with `data-lily-*-root` hooks, and a client.js
