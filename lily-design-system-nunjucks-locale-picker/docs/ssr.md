...
consumer passes `opts.value="fr"`, the matching radio gets
`checked` rendered server-side and the `<label lang="fr">` carries
the BCP 47 tag.
...
matching radio is checked from byte zero.
...
The picker renders without the client.js loading: a `<fieldset>`
with radios is a fully functional `<form>` control. Submit it
...
in JS-disabled mode, and curl users all see the radios.
...
The user picks a radio, clicks Apply, the server sets the cookie
...
- The server renders no `checked` (because `opts.value=""`), but
  the client picks a non-empty value from `localStorage`. The
  user briefly sees an unchecked group flash, then the right
  radio jumps to `checked` after JS runs.
- **Fix.** Resolve the locale server-side (cookie / header /
  URL / session) and pass it as `opts.value`. Then the radio is
  rendered `checked` from byte zero.
