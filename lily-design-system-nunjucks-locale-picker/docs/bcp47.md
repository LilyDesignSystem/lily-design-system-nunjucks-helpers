...
attribute on the document root. This page summarises the spec
and shows how to think about subtag composition.
...
The IANA Language Subtag Registry is the **single source of
truth** for every subtag. ISO standards are upstream of it, but
the registry is what implementations consult at runtime.
...
build the resolver yourself and pass the result in via
`opts.value`.
...
The two are otherwise interchangeable. This helper accepts
either form in your `opts.locales` array (treats them as
equivalent for matching) and emits the hyphen form to the `lang`
attribute. The `onChange` payload preserves your original form
...
world). They modify the language to mean "as used in this
region", not "spoken by this country's nationals".
...
catch-all when you don't want to ship 19 separate Latin
...
The Nunjucks macro can't call `Intl.DisplayNames` directly (it
runs at server-side render time), but you can populate
`opts.localeLabels` with `Intl.DisplayNames` output from your
Eleventy data file or Express middleware:

```js
// _data/localeLabels.js
const dn = new Intl.DisplayNames(["en"], { type: "language" });
const codes = ["en", "fr", "ar", "he", "ja", "zh-Hans", "zh-Hant"];
export default Object.fromEntries(codes.map((c) => [c, dn.of(c) ?? c]));
```

```njk
{{ localePicker({locales: codes, localeLabels: localeLabels}) }}
```
...
US-English-specific, use `en-US`. If they're general English,
use `en`. Don't overspecify: `en-US-x-stripe` is well-formed BCP
47 but hostile to negotiation.
...
implicitly Latin and `Suppress-Script: Latn` in the registry
says so.
