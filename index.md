...
| [`@lilydesignsystem/nunjucks-kanban-board`](./lily-design-system-nunjucks-kanban-board/)         | Interactive kanban board over the headless `kanban-table` macro family: pointer drag-and-drop plus a keyboard-accessible per-card "Move to…" menu, never drag-only. |
| [`@lilydesignsystem/nunjucks-gantt-chart`](./lily-design-system-nunjucks-gantt-chart/)           | Interactive Gantt chart over the headless `gantt-table` macro family: task bars as column-spanning cells, row hierarchy, milestones, and keyboard-accessible editing composed from two `date-time-picker` instances. |

`kanban-board` and `gantt-chart` are a seventh and eighth helper: both
are interactive layers over a headless `*-table` macro family rather
than preference/action/form-value controls in their own right.
`gantt-chart` is also the first helper besides `picker-bar` to depend
on a *sibling helper* (`date-time-picker`, used twice per edit
session) as a real package. See each package's own `spec/index.md` §3
for what each composes and why.
