...
If the consumer passes `opts.value="dark"`, the matching radio
...
renders the matching radio as `checked`; the client.js hydrates
...
- The server renders `value=""` (no radio checked), but the
  client picks a non-empty value from `localStorage`. The user
  briefly sees an unchecked group, then the chosen radio fills
  in.
