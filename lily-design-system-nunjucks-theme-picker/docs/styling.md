...
| `.theme-picker-icon`      | The `<span>` wrapping the default glyph. Absent when a `{% call %}` block overrides it.                                                                        |
| `.theme-picker-list`      | The `<ul role="listbox">`.                                                                                                                                     |
| `.theme-picker-option`    | Each `<li role="option">`.                                                                                                                                     |
| `.theme-picker-status`    | The consumer-rendered status region announcing the active theme (see [accessibility.md](./accessibility.md)). Ships in the examples; not emitted by the macro. |

The `.theme-picker-placeholder` hook is **gone**. There is no
...
| Selector                                      | Meaning                                                                |
| --------------------------------------------- | ---------------------------------------------------------------------- |
| `.theme-picker-list:not([hidden])`           | The listbox is open.                                                   |
| `.theme-picker-button[aria-expanded="true"]` | The button while its listbox is open.                                  |
| `.theme-picker-option[aria-selected="true"]` | The applied theme.                                                     |
| `.theme-picker-option[data-active]`          | The keyboard-active option (roving highlight). Distinct from selected. |
...
glyph only. `.theme-picker-icon` disappears; every other hook — root,
...
| `data-lily-theme-picker-root`     | the root `<div>`            | `autoInit()` selector.                      |
| `data-lily-theme-picker-button`   | the `<button>`              | Client lookup hook.                         |
| `data-lily-theme-picker-list`     | the `<ul>`                  | Client lookup hook.                         |
| `data-lily-theme-picker-input`    | the hidden `<input>`        | Client lookup hook.                         |
...
.theme-picker-option[aria-selected="true"]::before {
  content: "\2713\00a0"; /* check mark + nbsp */
}

.theme-picker-option:not([aria-selected="true"])::before {
...
```css
.utility-banner .theme-picker-button {
  border: 2px solid transparent;
}

.utility-banner .theme-picker-button:focus-visible {
...
- **Don't override `hidden` with a `display` rule.** A bare
  `.theme-picker-list { display: block }` beats the `hidden`
