...
function setup(
  ...
  const controller = initThemePicker(root, initOpts);
...
describe("ThemePicker — server-rendered listbox state (§7.14–§7.16)", () => {
...
describe("ThemePicker — keyboard contract (APG listbox, §7.20–§7.24)", () => {
...
describe("ThemePicker — system-preference detection (§7.27)", () => {
  ...
  test("§7.27 the macro emits the detect flag; detection never affects server markup", () => {
    ...
    expect(root.getAttribute("data-lily-theme-picker-detect-from-system")).toBe(
      "true",
    );
...
describe("ThemePicker — initial-value precedence (§7.28)", () => {
...
describe("ThemePicker — client.js lifecycle (§7.7–§7.11, §7.13)", () => {
  ...
  test("§7.17 opts.value is carried on data-lily-theme-picker-value and resolves the initial theme", () => {
    const root = mountIntoBody(
      renderMacro({
        label: "Theme",
        themesUrl: URL_TRAILING,
    ...
    expect(root.getAttribute("data-lily-theme-picker-value")).toBe("abyss");
  ...
  test("§7.18 the value data attribute is omitted entirely when opts.value is unset", () => {
    ...
    expect(root.hasAttribute("data-lily-theme-picker-value")).toBe(false);
  });
