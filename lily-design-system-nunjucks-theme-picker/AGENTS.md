...
- `themeName(slug)` mirrors locale-picker's `localeName(code)`:
  ...
  two in agreement.
- `matchSystemTheme(themes)` mirrors locale-picker's
...
The macro emits a `<div class="theme-picker">` carrying
`data-lily-theme-picker-*` hooks describing the control's name, themes
URL, extension, storage key, default value, and — when `opts.value` is
set — the consumer's initial value (`data-lily-theme-picker-value`).
Inside it are a hidden input, an icon `<button>`, and a
`<ul role="listbox" hidden>`. On `initThemePicker(root)`, the client
...
> `<link rel="stylesheet" data-lily-theme-picker="{name}">` whose href is
...
```html
<div class="theme-picker {classes}" data-lily-theme-picker-root …>
  <input
    type="hidden"
    name="{name}"
    value="{selected}"
...
travels ONLY on `data-lily-theme-picker-value`.

Ids are `{id}-list` / `{id}-option-{i}` where `id` defaults to
`theme-picker-{name}`. Deterministic and SSR-safe; two instances
sharing a `name` need an explicit distinct `id`.

There is **no** `placeholder` param and **no**
`.theme-picker-placeholder` hook — both were removed with the
`<select>`.
