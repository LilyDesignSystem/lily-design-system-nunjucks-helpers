...
//    itself (arrow move/clamp, Home/End, typeahead, PageUp/PageDown,
//    Enter/Space/Escape/Tab) is delegated to the shared
//    @lilydesignsystem/nunjucks-listbox-behavior module, factored out
//    once four pickers (theme/locale/text-size/motion) each hand-rolled
//    the same ~150 lines of it. None of this exists in the server
//    markup — the button is inert until this module runs. See docs/ssr.md.
//
// B. The theme LIFECYCLE (unchanged):
//   0. Read the consumer's `value` prop from
//      `data-lily-theme-picker-value`. This is still the only channel
 ...
 *
 * Mirrors `localeName` in locale-picker. This is the JS statement of the
...
/**
 * Wire one rendered ThemePicker root.
 *
 * @param {HTMLElement} root - The <div data-lily-theme-picker-root>.
...
export function initThemePicker(root, opts = {}) {
  ...
  const button = root.querySelector("[data-lily-theme-picker-button]");
  const list = root.querySelector("[data-lily-theme-picker-list]");
  const input = root.querySelector("[data-lily-theme-picker-input]");
  ...
  const detectFromSystem =
    root.getAttribute("data-lily-theme-picker-detect-from-system") === "true";
  ...
  // 1. value prop — read from `data-lily-theme-picker-value`.
  initial = valueAttr;
...
function warnResolutionOrderChanged(root, value, stored) {
  try {
    ...
    console.warn(
      `[lily theme-picker] value="${value}" now takes precedence over ` +
