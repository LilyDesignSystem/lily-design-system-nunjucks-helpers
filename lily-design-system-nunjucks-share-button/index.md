# Lily Design System — Nunjucks ShareButton
...
```sh
npm install lily-design-system-nunjucks-share-button
```
...
targets to `initShareButton` / `autoInit` and every anchor is rebuilt
...
| `id` | string | no | `share-button-{name}` | Id prefix. Needed when two instances share a `name`. |
...
} from "lily-design-system-nunjucks-share-button";
```

`initShareButton(root, opts?)` accepts `url`, `title`, `text`,
...
```njk
{% call shareButton({label: "Share this article", targets: targets}) %}
...
| Class | Element |
| ----- | ------- |
| `.share-button` | root `<div>` |
| `.share-button-trigger` | the `<button>` (**not** `-button`, which would read `.share-button-button`) |
| `.share-button-icon` | the glyph `<span>` |
| `.share-button-list` | the `<ul>`, `hidden` when closed |
| `.share-button-list-item` | each `<li>` |
| `.share-button-target` | each destination `<a>` |
| `.share-button-copy` | the copy `<button>` |
