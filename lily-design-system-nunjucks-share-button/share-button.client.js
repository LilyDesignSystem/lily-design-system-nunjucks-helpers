// ShareButton client-side runtime.
//
// Pairs with share-button.njk. The macro renders the markup with
// `data-lily-share-button-*` hooks and REAL destination hrefs; this
 ...
 * the macro's default `share-button-{name}` shape. The macro derives
 ...
 * roots in JavaScript.
 */
export function nextShareButtonId() {
    uid += 1;
...
/**
 * Wire one rendered ShareButton root.
 *
 * @param {HTMLElement} root - The <div data-lily-share-button-root>.
 * @param {{
 *   url?: string,
 *   title?: string,
 *   text?: string,
 ...
 */
export function initShareButton(root, opts = {}) {
    const noop = {
        open: () => {},
        close: () => {},
    ...
    const trigger = root.querySelector("[data-lily-share-button-trigger]");
    const list = root.querySelector("[data-lily-share-button-list]");
    const status = root.querySelector("[data-lily-share-button-status]");
    if (!trigger || !list) return noop;

    const attr = (suffix) =>
        root.getAttribute(`data-lily-share-button-${suffix}`) || "";
    ...
    function items() {
        return Array.from(
            list.querySelectorAll(".share-button-target, .share-button-copy"),
        );
    }
    ...
    function refreshHrefs() {
        ...
        for (const target of opts.targets) {
            ...
            const anchor = list.querySelector(
                `.share-button-target[data-target-id="${target.id}"]`,
            );
            if (!anchor) continue;
    ...
    function onListClick(event) {
        ...
        const copy = el.closest(".share-button-copy");
        if (copy) {
            copyUrl();
            return;
        }

        const anchor = el.closest(".share-button-target");
        if (!anchor) return;
...
/**
 * Find every [data-lily-share-button-root] and wire it.
 *
 * @param {Parameters<typeof initShareButton>[1]=} opts
 * @returns {Array<ReturnType<typeof initShareButton>>}
 */
export function autoInit(opts = {}) {
    if (typeof document === "undefined") return [];
    const roots = Array.from(
        document.querySelectorAll("[data-lily-share-button-root]"),
    );
    return roots.map((root) => initShareButton(root, opts));
}
