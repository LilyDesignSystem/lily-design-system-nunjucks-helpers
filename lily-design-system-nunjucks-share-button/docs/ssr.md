...
at render time; the browser parses and paints it; `share-button.client.js`
...
**The destination links work.** Every `<a class="share-button-target">`
...
- **Copy does not function.** `<button class="share-button-copy">` is a
...
`theme-select`, `locale-select` and `text-size-select` each render a
...
{% macro alwaysOpenShare(opts) %}
  {{ shareButton(opts) | replace(' hidden data-lily-share-button-list', ' data-lily-share-button-list') | safe }}
{% endmacro %}
```

Then do not call `initShareButton` on it. You lose the native sheet and
...
2. **The share URL**, used by the native sheet, by copy-to-clipboard,
   ...
   `opts.url` (init) → `data-lily-share-button-url` (macro) → `location.href`.
...
```njk
{{ shareButton({label: "Share", targets: t, name: "header"}) }}
{{ shareButton({label: "Share", targets: t, name: "footer"}) }}
```
