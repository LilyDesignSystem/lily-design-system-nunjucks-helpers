...
- **Voice-control users must guess.** Someone saying "click Share" is
  ...
  ```njk
  {% call shareButton({label: "Share this article", targets: t}) %}
...
`<p class="share-button-status" aria-live="polite">` is empty on load
...
including `.share-button-status:empty { display: none }` — do **not**
