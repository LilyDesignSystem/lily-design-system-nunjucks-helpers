# AGENTS — ShareButton (Nunjucks helper)
...
| `share-button.njk` | Nunjucks macro (`shareButton(opts)`). |
| `share-button.client.js` | ES module — `initShareButton`, `autoInit`, helpers. |
| `share-button.test.ts` | Vitest spec, mapped to the §7 clauses. |
| `index.md` | User guide. |
...
- Import: `{% from "./share-button.njk" import shareButton %}`
- Call: `{{ shareButton({label, targets, …}) }}`
...
1. **On the client.** `initShareButton(root, {targets})` accepts
...
URL is resolved lazily: init opt → `data-lily-share-button-url` →
...
`<div class="share-button" data-lily-share-button-root>` →
`<button class="share-button-trigger">` with an `aria-hidden` glyph span
→ `<ul class="share-button-list" hidden>` of `<li>` containing
`<a class="share-button-target">` and an optional
...
where `id` defaults to `share-button-{name}` and `name` defaults to
