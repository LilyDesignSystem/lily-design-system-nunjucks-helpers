# Changelog — SharePicker (Nunjucks)
...
`.share-picker-button`, matching `theme-picker`, `locale-picker` and
...
- **`share-picker.njk`** — the `sharePicker(opts)` macro. Renders a
  ...
  `<button class="share-picker-button">` (U+27A4), a
  `<ul class="share-picker-list" hidden>` of destination `<a>` elements
  plus an optional copy `<button>`, and a polite
  `<p class="share-picker-status">` live region. Ids are deterministic,
  derived from `opts.name` (default `"share"`) or an explicit `opts.id`.
- **`share-picker.client.js`** — the runtime. Owns open/close, focus
  ...
  `nextSharePickerId`, `shareTargetHref`, and
  `BLACK_RIGHTWARDS_ARROWHEAD`.
- **Owns an action, not a preference** — applies nothing to the document
  root and persists nothing.
- **`share-picker.test.ts`** — 55 vitest cases mapped 1:1 onto the §7
...
- **`share-picker.njk`** — the `sharePicker(opts)` macro. Renders a
  `<div class="share-picker">` root containing a glyph-only
  `<button class="share-picker-button">`, a `<ul class="share-picker-list" hidden>`
  of destination `<a>` elements plus an optional copy `<button>`, and a
  polite `<p class="share-picker-status">` live region. Ids are
  deterministic, derived from `opts.name` (default `"share"`) or an
  explicit `opts.id`.
- **`share-picker.client.js`** — the runtime. Owns open/close, real
...
3. The _contract_ is unchanged: the consumer still owns the whole URL,
...
Unlike `theme-picker`, `locale-picker` and `text-size-picker`, which are
...
- **The list cannot be opened.** It is rendered `hidden` and only the
  client removes that, so the links are unreachable _through the
  control_.
...
apply a _preference_ to the document, which is inherently a runtime act,
while this helper's primary affordance is _navigation_, which HTML does
...
- **Nothing is persisted and nothing is applied to the document.** This
  is the first helper in the catalog that owns an _action_ rather than a
