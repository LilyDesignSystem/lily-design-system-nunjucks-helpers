...
// ---------------------------------------------------------------------
// Nunjucks env that can resolve `./share-picker.njk` from this dir.
// ---------------------------------------------------------------------
...
function renderMacroWithCaller(
  ...
  const src =
    `{% from "./share-picker.njk" import sharePicker %}` +
    `{% call sharePicker(opts) %}${body}{% endcall %}`;
  return env.renderString(src, { opts });
}
...
type Parts = {
  ...
  api: ReturnType<typeof initSharePicker>;
};
...
function setup(
  ...
  const api = initSharePicker(root, initOpts);
  return {
    root,
    trigger: root.querySelector(".share-picker-button") as HTMLButtonElement,
    list: root.querySelector(".share-picker-list") as HTMLElement,
    status: root.querySelector(".share-picker-status") as HTMLElement,
    targets: Array.from(
      root.querySelectorAll<HTMLAnchorElement>(".share-picker-target"),
    ),
    copy: root.querySelector(".share-picker-copy"),
    api,
  };
}
...
describe("SharePicker — markup contract (§7.1–§7.6)", () => {
  ...
  test("§7.1 the trigger class is share-picker-button, matching the other helpers", () => {
    ...
    expect(trigger.className).toBe("share-picker-button");
    expect(root.querySelector(".share-picker-button")).toBe(trigger);
    // The pre-rename `share-button-trigger` exception is gone.
    expect(root.querySelector(".share-picker-trigger")).toBeNull();
  });
  ...
  test("§7.3 each destination sits in its own list item", () => {
    ...
    const lis = root.querySelectorAll(".share-picker-list-item");
    expect(lis.length).toBe(2);
    for (const a of targets) {
      expect(a.parentElement?.className).toBe("share-picker-list-item");
...
describe("SharePicker — keyboard and dismissal (§7.15–§7.19)", () => {
  ...
  test("§7.18 choosing a destination fires onShare with its id and closes", () => {
    const onShare = vi.fn();
    const { trigger, list, root } = setup({ url: URL_UNDER_TEST }, { onShare });
    click(trigger);
...
describe("SharePicker — url resolution and glyph (§7.20–§7.22)", () => {
  ...
  test("§7.20 the url travels on a data attribute, not a rendered href", () => {
    ...
    expect(root.getAttribute("data-lily-share-picker-url")).toBe(
      URL_UNDER_TEST,
    );
    ...
    expect(bare.hasAttribute("data-lily-share-picker-url")).toBe(false);
  });
  ...
  test("§7.22 a {% call %} body replaces the glyph", () => {
    ...
    expect(custom.closest("button")?.className).toBe("share-picker-button");
...
describe("SharePicker — Nunjucks surface (§7.23–§7.26)", () => {
  test("§7.23 ids are deterministic and derived from the default name", () => {
    const { trigger, list } = setup();
    expect(list.id).toBe("share-picker-share-list");
  ...
  test("§7.23 the name opt drives the ids", () => {
    ...
    expect(list.id).toBe("share-picker-footer-list");
    expect(root.getAttribute("data-lily-share-picker-name")).toBe("footer");
    expect(root.querySelector(".share-picker-target")!.id).toBe(
      "share-picker-footer-target-0",
    );
  });
  ...
  test("§7.23 an explicit id opt overrides the name-derived prefix", () => {
    ...
    expect(root.querySelector(".share-picker-copy")!.id).toBe(
      "custom-prefix-copy",
    );
  });
  ...
  test("§7.23 two instances sharing a name collide, distinct ids do not", () => {
    ...
    const lists = Array.from(
      document.querySelectorAll(".share-picker-list"),
    ).map((l) => l.id);
    expect(new Set(lists).size).toBe(2);
    expect(lists).toEqual(["share-picker-share-list", "b-list"]);
  });
  ...
  test("§7.26 autoInit wires every root on the page", () => {
    ...
    const triggers = Array.from(
      document.querySelectorAll<HTMLButtonElement>(".share-picker-button"),
    );
    click(triggers[1]);
    const lists = Array.from(
      document.querySelectorAll<HTMLElement>(".share-picker-list"),
    );
    expect(lists[0].hasAttribute("hidden")).toBe(true);
    expect(lists[1].hasAttribute("hidden")).toBe(false);
  });

  test("§7.26 initSharePicker is inert on a missing or foreign root", () => {
    expect(() => initSharePicker(null as any)).not.toThrow();
    document.body.innerHTML = "<div></div>";
    const api = initSharePicker(
  ...
  test("§7.26 nextSharePickerId mints stable, incrementing, SSR-safe ids", () => {
...
describe("SharePicker — no-JS degradation (§6)", () => {
  test("§6 destination links are complete in the server markup", () => {
    // No initSharePicker call at all — this is the no-JS document.
    const root = mountIntoBody(
      renderMacro({
        label: "Share",
        targets: TARGETS,
  ...
  test("§6 the list stays hidden and copy is inert without the client", () => {
    ...
    const list = root.querySelector(".share-picker-list") as HTMLElement;
    click(trigger);
    expect(list.hasAttribute("hidden")).toBe(true);
    expect(trigger.getAttribute("aria-expanded")).toBe("false");
  });
